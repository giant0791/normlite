...
def test_columns_subset_projection(students: Table):
    ...
    assert compiled.fetch_columns() == ['object_id', 'name', 'id', 'is_active']
...
def test_explicit_object_id_exists_once_in_fetch_columns(students: Table):
    ...
    assert asdict['query_params']['filter_properties'] == expected
    assert compiled.result_columns() == expected
    assert compiled.fetch_columns() == list(itertools.chain(["object_id"], expected))
...
def test_syscol_subset_in_fetch_columns_no_result_columns(students: Table):
    ...
    assert compiled.result_columns() == expected
    assert compiled.fetch_columns() == ["is_archived", "is_deleted", "created_at"]
...
def test_columns_all_projection(students: Table):
    ...
    all_expected = list(itertools.chain(sys_expected, usr_expected))
