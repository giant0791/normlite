...
- **notion_sdk**: persist two-object store; reject pre-2025-09-03 files
- **notion_sdk**: search data sources by title for orphan detection
- **notion_sdk**: narrow databases.update to container-level attrs
- **notion_sdk**: advertise data source name on the container
- **sql**: route reflection catalog-first onto the data source
- **engine**: persist database created_time on the tables catalog row
- **sql**: backfill user-column ids on create via shared data-source reflection
- **sql**: exclude data_source_id from the public column surface
- **notion_sdk**: retire the databases.query endpoint
