...
from normlite.exceptions import CompileError, UnsupportedCompilationError
...
class RowCompiler(NotionObjectCompiler):
    ...
    def visit_property(self, prop: NotionProperty) -> tuple:
        if prop._is_page_property:
            ...
            # the property being visited belongs to a database object
            # adapt to return a row with the columns: name, typecode, id, value
            return (
                prop.name, 
                _typecode_mapper[prop.type],
                prop.id, 
                prop.value,
            )
...
class DescriptionCompiler(NotionObjectCompiler):
    ...
    def visit_database(self, database: NotionDatabase) -> Sequence[tuple]:
        return [
            self._add_not_used_seq(('column_name', DBAPITypeCode.META_COL_NAME,)), 
            self._add_not_used_seq(('column_type', DBAPITypeCode.META_COL_TYPE,)),
            self._add_not_used_seq(('column_id', DBAPITypeCode.META_COL_ID,)),
            self._add_not_used_seq(('column_value', DBAPITypeCode.META_COL_VALUE,)),
        ]
